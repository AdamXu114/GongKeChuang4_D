...
image2_t image2_gauss, image2_binary;
...
void hsp_image2_fast_gauss_conv(image2_t input, image2_t output)
{
	uint16_t r, c;
...
void hsp_image_sobel2(image_t A, image_t B)
{
	...
	for (i=1; i<(IMAGEH-1); i++)
	{
		for (j=1; j<(IMAGEW-1); j++)
		{
			...
			B[i][j] = 255-fast_sqrtf(Xg*Xg+Yg*Yg);	// 
		}
	}
}
...
// https://www.cnblogs.com/ZYQS/p/14665190.html
void hsp_image2_sobel1(image2_t A, image2_t B)
{
	uint16_t i, j;
	int Xg = 0; 	// 横向梯度值
	int Yg = 0; 	// 纵向梯度值
...
// https://shenchuan.blog.csdn.net/article/details/85250924
void hsp_image2_sobel2(image2_t A, image2_t B)
{
	uint16_t i, j;
	int Xg = 0; 	// 横向梯度值
	int Yg = 0; 	// 纵向梯度值
...
void hsp_image2_canny(image2_t A, image2_t B, uint8_t low_thres, uint8_t high_thres)
{
	uint16_t i, j, x, y;
	static image_t CannyAm, Sector;
	int Xg = 0;		// 横向梯度值
	...
	for(y=1; y<(IMAGEH2-1); y++)
	{
		for(x=1; x<(IMAGEW2-1); x++)
		{
			switch(Sector[y][x])
			{
			case 0:
